...
private:
    ...
    void loop()
    {
        while (true)
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(delayLoop));
            pacAnim += .4;
    ...
    void animation()
    {
        ...
        {
            int bonus = 100 * GameObject::level;
        ...
        sprOfsX = (sprW * static_cast<int>(pacAnim));
        pacSpr.setTextureRect(sf::IntRect(sprOfsX, sprOfsY, sprW, sprH));
        GameObject::pacPosition = &pacPos;
    }
    ...
    void eatDot(int pacX, int pacY)
    {
        for (int i = 0; i < maze->dotsCount; i++)
        {
            ...
            int dotX = static_cast<int>(pos.x / sprW);
            int dotY = static_cast<int>(pos.y / sprH);
    ...
    bool xyModul()
    {
        modulX = static_cast<int>(pacSpr.getPosition().x) % sprW;
        modulY = static_cast<int>(pacSpr.getPosition().y) % sprH;
